...
def augment_dataset(input_path="creditcardfraud/creditcard.csv", output_path="creditcard_1M.csv", target_rows=1_000_000):
    ...
    # Separate fraud and normal so we can augment both properly
    df_normal = df[df['Class'] == 0]
    df_fraud = df[df['Class'] == 1]
    
    # Function to create "nudged" data
    def augment_data(dataframe, num_new_rows):
        new_rows = []
        features = dataframe.drop('Class', axis=1)
        
        for _ in range(num_new_rows):
            # Pick a random original row
            base_row = features.sample(1).iloc[0]
            
            # Add a tiny bit of random noise to the features (except Time)
            noise = np.random.normal(0, 0.01, size=len(base_row) - 1)
            new_row = base_row.copy()
            new_row[1:] = new_row[1:] + noise  # Add noise to V1-V28 and Amount
            
            new_rows.append(new_row)
            
        new_df = pd.DataFrame(new_rows, columns=features.columns)
        new_df['Class'] = dataframe['Class'].values[0] # Keep the original label
        return new_df

    # Augment normal and fraud separately to keep the ratio
    ...
    # Shuffle it up so fraud isn't all at the bottom
